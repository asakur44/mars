...
`ask_grok` defaults to `grok-4.5` (xAI flagship; default bumped 2026-08-04 — fall back to `grok-4.3` if 4.5 errors). Other choices:

```python
ask_grok(prompt="...", model="grok-4.3")                  # prior flagship / fallback (1M ctx)
...
- `model`: default `"grok-4.5"` (xAI flagship; default bumped 2026-08-04). Fallback `"grok-4.3"` (prior flagship; leads on non-hallucination rate, agentic tool-calling, instruction following; 1M ctx; $1.25/$2.50 per M, $0.20/M cached input). Prior flagship `"grok-4.20-reasoning"` ($2/$6 per M). Drop to `"grok-4-1-fast-reasoning"` ($0.20/$0.50 per M, 2M ctx) for high-volume work. Other ids: `"grok-code-fast-1"` (256K coding-tuned), `"grok-4"` / `"grok-4-0709"` (older 256K).
